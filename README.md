...
A standalone pocket groovebox I built for the **M5Stack Cardputer ADV**: an
8-voice synth, a drum machine, a step sequencer, an arranger and effects. It
needs no phone, no DAW and no MIDI host to do anything. I play it on my own
ADV.
...
I read everything below out of the installed M5Unified / M5Cardputer sources and
the ESP32 board definition rather than assuming it from the original Cardputer.
...
one FQBN covers both machines. I refuse to run on a non-ADV board rather than
...
Host tests and tools. Everything below `app.cpp` except the Arduino-facing files
builds and runs on a desktop, which is what lets me measure the DSP rather than
estimate it.
...
`all-tests` is two suites I wrote. The first covers the DSP, sequencer timing, music
theory and the file format, and now the sound itself: kick level through a
200 Hz high-pass, lane balance against the kick, choke timing,
...
16-entry palette, which are the two properties the live legend and the 4-bit
canvas rest on.

`render-wav` covers what an assertion cannot. Whether a kick sounds right is not
something a test decides, so the renderer writes WAVs for me to listen to and
prints the measurements alongside them. Every sound bug I fixed in this round I
found that way, and each one is now pinned by a test.
...
You do not have to memorise any of that. Hold `FN` or `SHIFT` for a moment and
the screen lists everything that modifier does on the screen you are on; let go
and you are playing again. I generate that list by walking the same function the
firmware dispatches through, so it cannot describe a binding that does not exist
or miss one that does, and I generate `CONTROLS.md` from the same tables so it
stays in step as well.

## Modes

| # | Mode | What it is |
| --- | --- | --- |
| 1 | **PLAY** | Live keyboard, scope, cutoff/resonance under your fingers, drum pads on the number row. This is the screen it boots into. |
...
- *PLUCK*: Karplus-Strong, with an excitation shaped by brightness and pick
...
I put each engine's own controls in a 20-slot overlay on the parameter array, so
...
I built the kick around a property of this hardware: the Cardputer's speaker has
almost no output below ~200 Hz, so a kick whose weight lives in its fundamental
is inaudible on the device however well it is tuned. I get the weight instead
from a two-stage pitch envelope sweeping through 350–150 Hz and from saturation
putting harmonics there, with a clean sub layer underneath for headphones. My
host renderer measures exactly that, peak level surviving a 200 Hz high-pass,
for every kit.
...
`SPEAKER` / `LINE` output modes matter more than they look like they would. On
`SPEAKER` I high-pass the master at 120 Hz, because energy the speaker cannot
reproduce was otherwise still driving the limiter, and a big kick would duck the
whole mix in exchange for something nobody can hear.
...
- **32 kHz mono, 128-sample blocks (4 ms).** The ES8311 runs at its 48 kHz
  default and M5Unified box-resamples our blocks up to it. That is the same path
  the MP3 players on this board use, and one less clocking assumption to get
  wrong.
...
- I shrink the I²S DMA ring to 4 × 256 frames (~21 ms) and pin the speaker task
  to core 0 at priority 5, above the render task at 4.
...
- I advance the sequencer and arpeggiator **in sample counts, not blocks**, and
  split each block at every step and gate boundary, so step timing is
...
I put projects on the SD card at `/synthcard/<NAME>.SCP`, and nowhere else. I
create the directory the first time you save and leave the card alone during
normal play; I mount it for the transfer and unmount it immediately.
...
bytes. My reader is version-aware and still loads v2 files: I remap the drum lanes
(v2's lane 4 was CLAP, which is lane 6 now, so copying by index would silently
turn every saved clap into a ride) and convert patches parameter by parameter. I
prove that against a hand-built v2 image rather than by assertion. Loading validates the magic, the version, the checksum, the
...
resident for the whole session to be used for a few milliseconds at a time. The
caller now lends the undo buffer instead. It is the same size and expendable at
exactly that moment, since loading a project clears the undo history anyway. The
format packs each step's gate and chord into one byte, which is what keeps a
project inside a borrowed buffer. A test asserts a fully populated project still fits.

I suspend audio around every card access: the SPI transfer and FAT bookkeeping
are far too long for the render task.

## Performance notes

`make -C test bench` reports per-engine and per-lane cost. The absolute numbers
do not transfer, since a desktop is not an LX7, but the ratios do, and so does
watching them move between commits. The six engines land between 1.0× and
...
The benchmark paid for itself immediately: a sustained Karplus-Strong string
measured 18× an ANALOG voice, where the real figure is 1.2×. The difference was
denormals. After about nine seconds the loop's filter states decay past 1e-38,
and denormal arithmetic is handled in software. I now flush filter and loop
states once per block throughout the engine.

**Memory.** I got static RAM down to 105 KB from 122 KB, despite going to six
engines from four and twelve drum lanes from nine. The canvas is a 4-bit palette sprite: 16.2 KB from the heap instead of
...
worse one. A step is four bytes rather than five, since gate needs five bits and
chord needs two so they share a byte, which saves 2 KB across the project and
its undo copy.
...
I cap the UI at ~33 fps so the display push never competes with audio.
...
- **No MIDI.** Out of scope for v1; the standalone instrument came first.
...
- **Polyphony under the heavier engines.** ORGAN and FM cost roughly 2.3× an
  ...
  shedding down towards three. That is how I designed it rather than a dropout,
  but it is a real ceiling.
- **Unison spends polyphony.** A 4× supersaw patch gives two notes rather than
  eight, because I charge unison as voice slots rather than asking the CPU for
  four times the work.
## Licence
